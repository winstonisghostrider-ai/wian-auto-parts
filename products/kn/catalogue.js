(() => {
  ...
  const DATA_URL = "/data/kn-products.json?v=20260906";
  const PRODUCT_IMAGES_URL = "/data/kn-product-images.json?v=20260906";
  const WHATSAPP_NUMBER = "919108327761";
  const state = { products: [], imageManifest: new Map(), query: "", makes: new Set(), statuses: new Set(), sort: "relevance" };
  ...
  function safeImageStem(partNumber) {
  ...
  function expectedImagePath(product) {
    return `/assets/products/kn/${safeImageStem(product.p)}.webp`;
  }
  ...
  function imageMarkup(product) {
    ...
    if (source) {
      return `<div class="product-media"><img class="product-image" src="${escapeHtml(source)}" alt="K&amp;N performance air filter ${escapeHtml(product.p)}" loading="lazy" decoding="async" data-part="${escapeHtml(product.p)}"></div>`;
    }
    return pendingImageMarkup(product);
  }

  function pendingImageMarkup(product) {
    return `<div class="product-media" data-expected-image="${escapeHtml(expectedImagePath(product))}"><div class="image-pending"><div><strong>Product image pending verification</strong><span>Exact ${escapeHtml(product.p)} product image required</span></div></div></div>`;
  }
  ...
  function cardMarkup(product) {
    ...
    return `<article class="product-card" data-part="${escapeHtml(product.p)}">
      ...
      <div class="product-body">
        ...
        <p class="vehicle-note">Vehicle photo pending verification</p>
  ...
  function bindProductActions() {
    ...
    $$(".product-image", elements.grid).forEach((image) => {
      image.addEventListener("error", () => {
        ...
        if (product) image.closest(".product-media").outerHTML = pendingImageMarkup(product);
      }, { once: true });
    });
  }
  ...
  function openDetails(partNumber) {
    ...
    const rows = applications(product).map((application) => {
      ...
      return `<div class="fitment-row">
        <div class="vehicle-placeholder" aria-label="Vehicle photo pending verification">Photo pending</div>
